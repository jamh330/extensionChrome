...
function buildHTMLfromStorage(){
	chrome.storage.sync.get(function(storage){
		...
		for(key in items){
			html += '<p class="'+key+'">'+
				'<input type="text" placeholder="Titulo" class="item-title" value="'+items[key]['item-title']+'"/>'+
				'<input type="text" placeholder="Contenido" class="item-content" value="'+items[key]['item-content']+'"/>'+
...
function createHTMLItem(){
	...
	elem.innerHTML ='<input type="text" placeholder="Titulo" class="item-title"/>'+
					'<input type="text" placeholder="Contenido" class="item-content" />'+
					'<button class="remove" title="Click to remove this snippet">&times;</button>';
